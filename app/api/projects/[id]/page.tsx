...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          console.warn("프로젝트 상태 로드 실패:", data?.error);
          return;
        }
        ...
        if (state.survey) {
          setBudget(state.survey.budget || "");
          setTimeline(state.survey.timeline || "");
          setTargetMarket(state.survey.target_market || "");
          setPriority(state.survey.priority || "");
          setRiskTolerance(state.survey.risk_tolerance || "");
          setRegulationFocus(state.survey.regulation_focus || "");
        }

        if (state.user_notes) {
          setUserNotes({
            target_problem: state.user_notes.target_problem || "",
            key_features: state.user_notes.key_features || "",
            differentiation: state.user_notes.differentiation || "",
            concept: state.user_notes.concept || "",
          });
        }

        if (state.rfp_json) {
          setRfp(state.rfp_json);
        }
      } catch (e) {
        console.error("프로젝트 상태 로드 중 오류:", e);
      }
    })();
  }, [projectId]);
  ...
  // ✅ 현재 상태를 Supabase에 저장하는 헬퍼
  async function persistProjectState(partial?: { step?: number }) {
    if (!projectId) return;
    ...
    try {
      ...
      console.error("프로젝트 상태 저장 중 오류:", e);
    }
  }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      // ✅ 프로젝트 상태 저장 (step 3: RFP 생성 완료)
      await persistProjectState({ step: 3 });
    } catch (e: any) {
  ...
  async function handleRefineRfp() {
    ...
    try {
      ...
      // ✅ 프로젝트 상태 저장 (step 4: RFP 리파인)
      await persistProjectState({ step: 4 });
    } catch (e: any) {
  ...
  async function handleGenerateDesign() {
    ...
    try {
      ...
      // ✅ 프로젝트 상태 저장 (step 6: 디자인 시안 생성 완료)
      await persistProjectState({ step: 6 });
    } catch (e: any) {
  ...
  async function handleGenerateConceptImages() {
    ...
    try {
      ...
      // ✅ 프로젝트 상태 저장 (step 5: 컨셉 이미지 생성)
      await persistProjectState({ step: 5 });
    } catch (e: any) {
